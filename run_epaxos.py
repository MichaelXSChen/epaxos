...
n_reqs = 2000 # number of requests
...
for i in range (N):
    command = 'sh -c \'/app/bin/paxos-server -maddr %s -mport %s -addr %s -peerEPort %s -managerEPort %s -e >/logs/server%d.log 2>&1\'' % (master_ip, 7087, server_ips[i], 7070, 8070, i)
